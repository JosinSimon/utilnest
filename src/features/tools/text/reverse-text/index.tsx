...
const MODES: { mode: ReverseMode; label: string }[] = [
  { mode: "characters", label: "Characters" },
  { mode: "words", label: "Words" },
  { mode: "lines", label: "Lines" },
]

export default function ReverseText({ tool }: { tool: ToolDefinition }) {
    ...
    <Card>
      <CardContent className="p-6">
        ...
        <textarea
          ...
          placeholder="Paste text to reverse here…"
        ...
        <div className="mt-5 rounded-xl border bg-secondary/40 p-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium">Reversed · {mode}</p>
            <button
              type="button"
              onClick={onCopy}
          ...
          <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap break-words font-sans text-sm">
            {output || <span className="text-muted-foreground">Reversed text appears here.</span>}
          </pre>
        </div>
      </CardContent>
    </Card>
